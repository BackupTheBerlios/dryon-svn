...
#define NILorSTRING(obj)	(NIL_P(obj)? NULL: StringValueCStr(obj))
...
{
    MYSQL* m = GetHandler(obj);
    if (mysql_create_db(m, StringValueCStr(db)) != 0)
	mysql_raise(m);
    return obj;
}
...
{
    MYSQL* m = GetHandler(obj);
    if (mysql_drop_db(m, StringValueCStr(db)) != 0)
	mysql_raise(m);
    return obj;
}
#endif
...
static VALUE list_fields(int argc, VALUE* argv, VALUE obj)
{
    ...
    res = mysql_list_fields(m, StringValueCStr(table), NILorSTRING(field));
    if (res == NULL)
	mysql_raise(m);
