...
/*
int get_packet_size(int s)
{
	int i;

	if ( ioctl(s, SIOCINQ, &i) == -1)
	{
		perror("Erreur ioctl");
		return -1;
	}
...
int get_info(string server_addr, server_info *s_info)
{
	...
	if( (n= server_addr.find(":")) != string::npos )
	{
		addr= server_addr.substr(0, n);
		port= atoi(server_addr.substr(n+1, server_addr.size()-n).c_str());
	}
	// addr
	else
	{
	...
	if( ping_host(addr.c_str(), port)==-1 )
		return -1;
	else
		return get_info(addr.c_str(), port, s_info);
}
*/
int get_infos(const char *server_addr, int port, server_info *s_info)
{
	...
	//s_info->country= GeoIP_country_name_by_addr(gi, server_addr);
...
/****f* GameServers_Extension/hl_ping
* USAGE
*	hl_ping(const host[], port= 27015)
* RETURN VALUE
