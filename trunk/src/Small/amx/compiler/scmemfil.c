 ...
 *
 *  Copyright (c) ITB CompuPhase, 2003-2004
 *
 ...
 *
 *  Version: $Id: scmemfil.c,v 1.1 2004-03-05 16:53:05+01 thiadmer Exp thiadmer $
 */
...
unsigned int mfwrite(MEMFILE *mf,unsigned char *buffer,unsigned int size)
{
  ...
  while (mf->bufpos+size>numblocks*BUFFERSIZE) {
    /* append a block */
    MEMFILE *last;
    block=malloc(sizeof(MEMFILE));
    if (block==NULL)
      return 0;
    memset(block,0,sizeof(MEMFILE));
    block->buffer=malloc(BUFFERSIZE);
