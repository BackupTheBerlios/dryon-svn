 ...
 *
 *  Version: $Id: sc6.c,v 1.3 2004/08/07 16:56:34 anthalir Exp $
 */
#include <assert.h>
#include <stdio.h>
...
SC_FUNC void assemble(FILE *fout,FILE *fin)
{
  ...
  hdr.natives=hdr.publics + numpublics*sizeof(FUNCSTUBNT);
  hdr.libraries=hdr.natives + numnatives*sizeof(FUNCSTUBNT);
  hdr.pubvars=hdr.libraries + numlibraries*sizeof(FUNCSTUBNT);
  hdr.tags=hdr.pubvars + numpubvars*sizeof(FUNCSTUBNT);
  hdr.nametable=hdr.tags + numtags*sizeof(FUNCSTUBNT);
  ...
  count=0;
  for (sym=glbtab.next; sym!=NULL; sym=sym->next) {
    if (sym->ident==iFUNCTN && (sym->usage & uPUBLIC)!=0 && (sym->usage & uDEFINE)!=0)
    {
      assert(sym->vclass==sGLOBAL);
      ...
      #endif
      fseek(fout,hdr.publics+count*sizeof(FUNCSTUBNT),SEEK_SET);
  ...
  if (numnatives>0) {
    ...
    for (i=0; i<numnatives; i++) {
      ...
      func.nameofs=nameofs;
        ...
        align32(&func.nameofs);
      #endif
      fseek(fout,hdr.natives+count*sizeof(FUNCSTUBNT),SEEK_SET);
      sc_writebin(fout,&func,sizeof func);
      fseek(fout,nameofs,SEEK_SET);
  ...
  for (constptr=libname_tab.next; constptr!=NULL; constptr=constptr->next) {
    if (constptr->value>0) {
      ...
      func.nameofs=nameofs;
        ...
        align32(&func.nameofs);
      #endif
      fseek(fout,hdr.libraries+count*sizeof(FUNCSTUBNT),SEEK_SET);
      sc_writebin(fout,&func,sizeof func);
      fseek(fout,nameofs,SEEK_SET);
  ...
  for (sym=glbtab.next; sym!=NULL; sym=sym->next) {
    if (sym->ident==iVARIABLE && (sym->usage & uPUBLIC)!=0) {
      ...
      func.nameofs=nameofs;
        ...
        align32(&func.nameofs);
      #endif
      fseek(fout,hdr.pubvars+count*sizeof(FUNCSTUBNT),SEEK_SET);
      sc_writebin(fout,&func,sizeof func);
      fseek(fout,nameofs,SEEK_SET);
      sc_writebin(fout,sym->name,strlen(sym->name)+1);
      nameofs+=strlen(sym->name)+1;
  ...
  for (constptr=tagname_tab.next; constptr!=NULL; constptr=constptr->next) {
    if ((constptr->value & PUBLICTAG)!=0) {
      ...
      func.nameofs=nameofs;
        ...
        align32(&func.nameofs);
      #endif
      fseek(fout,hdr.tags+count*sizeof(FUNCSTUBNT),SEEK_SET);
      sc_writebin(fout,&func,sizeof func);
      fseek(fout,nameofs,SEEK_SET);
      sc_writebin(fout,constptr->name,strlen(constptr->name)+1);
      nameofs+=strlen(constptr->name)+1;
