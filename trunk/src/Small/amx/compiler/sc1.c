 ...
 *
 *  Version: $Id: sc1.c,v 1.3 2004/08/07 16:56:34 anthalir Exp $
 */
#include <assert.h>
#include <ctype.h>
#include <limits.h>
...
#define VERSION_INT 250

static void resetglobals(void);
static void initglobals(void);
static void setopt(int argc,char **argv,char *iname,char *oname,
                   char *ename,char *pname,char *rname,char *codepage);
...
static void declfuncvar(int tok,char *symname,int tag,int fpublic,int fstatic,
...
static cell initarray(int ident,int tag,int dim[],int numdim,int cur,
                      ...
                      int *errorfound);
static cell initvector(int ident,int tag,cell size,int fillzero,int *errorfound);
...
static void doexpr(int comma,int chkeffect,int allowarray,int mark_endexpr,
...
int sc_writeasm(void *handle,char *string)
{
  #if defined __MSDOS__ || defined SC_LIGHT
...
char *sc_readasm(void *handle, char *string, int maxchars)
{
  #if defined __MSDOS__ || defined SC_LIGHT
...
int sc_compile(int argc, char *argv[])
{
  ...
  setopt(argc,argv,inpfname,outfname,errfname,incfname,reportname,codepage);
  ...
  /* write a report, if requested */
    ...
    if (sc_makereport) {
      ...
      if (frep!=NULL) {
        dump_referrers(&glbtab,frep,inpfname);
        if (strlen(reportname)>0)
          fclose(frep);
      } /* if */
    } /* if */
 ...
 /* ??? for re-parsing the listing file instead of the original source
  ...
  if (!entry)
    ...
    if (errnum==0 && strlen(errfname)==0) {
      long stacksize=max_stacksize(&glbtab)+32; /* add a margin */
      if ((sc_debug & sSYMBOLIC)!=0 || verbosity>=2 || stacksize>=(long)sc_stksize) {
        if (stacksize<0)
          printf("Unknown stack requirements (recursive functions)\n");
        else if (stacksize>0)
          printf("Estimated stack usage: %ld cells (alloted size=%ld cells)\n",
                 stacksize,(long)sc_stksize);
      } /* if */
    } /* if */
  #endif
  ...
  for (arg=1; arg<argc; arg++) {
    ...
    if (isoption) {
      ...
      case 'C':
        ...
        break;
      case 'c':
        strncpy(codepage,ptr+1,MAXCODEPAGE);  /* set name of codepage */
      ...
      #if !defined SC_LIGHT
        parserespf(&argv[arg][1],iname,oname,ename,pname,rname,codepage);
      #endif
...
#define MAX_OPTIONS     100
  ...
  parseoptions(argc,argv,iname,oname,ename,pname,rname,codepage);
    ...
    if (argv[0]!=NULL) {
      ...
      if ((ext=strrchr(cfgfile,DIRSEP_CHAR))!=NULL) {
        *(ext+1)='\0';          /* strip the program filename */
        strcat(cfgfile,"sc.cfg");
      } else {
        strcpy(cfgfile,"sc.cfg");
      } /* if */
      if (access(cfgfile,4)==0)
        parserespf(cfgfile,iname,oname,ename,pname,rname,codepage);
    } /* if */
  #endif
  parseoptions(argc,argv,iname,oname,ename,pname,rname,codepage);
  if (strlen(iname)==0)
...
static void setcaption(void)
{
  sc_printf("Small compiler " VERSION_STR "\t\tCopyright (c) 1997-2004, ITB CompuPhase\n\n");
}
...
static void about(void)
{
  if (strlen(errfname)==0) {
    ...
    sc_printf("         -c<name> codepage name or number; e.g. 1252 for Windows Latin-1\n");
#if defined dos_setdrive
    ...
    sc_printf("         -t<num>  TAB indent size (in character positions, default=%d)\n",sc_tabsize);
    sc_printf("         -v<num>  verbosity level; 0=quiet, 1=normal, 2=verbose (default=%d)\n",verbosity);
    sc_printf("         -\\       use '\\' for escape characters\n");
    sc_printf("         -^       use '^' for escape characters\n");
    sc_printf("         -;[+/-]  require a semicolon to end each statement (default=%c)\n", sc_needsemicolon ? '+' : '-');
    sc_printf("         -([+/-]  require parantheses for function invocation (default=%c)\n", optproccall ? '-' : '+');
...
static int declloc(int fstatic)
{
  ...
  do {
    ...
    if (fstatic) {
      ...
      defsymbol(name,ident,sSTATIC,(cur_lit+glb_declared)*sizeof(cell),tag);
    } else {
...
static cell adjust_indirectiontables(int dim[],int numdim,int cur,cell increment,
                                     int startlit,constvalue *lastdim)
{
static int base;
  int d;
  cell accum;
  ...
  if (dim[cur+1]>0) {
    ...
    assert(lastdim!=NULL);
    accum=0;
    for (d=0,ld=lastdim->next; d<dim[cur]; d++,ld=ld->next) {
      assert(ld!=NULL);
  ...
  if (cur+2<numdim) {   /* are there at least 2 dimensions below this one? */
    ...
    for (d=0; d<dim[cur]; d++)
      increment+=adjust_indirectiontables(dim,numdim,cur+1,increment,-1,lastdim);
  } /* if */
  return accum;
}
...
static void initials(int ident,int tag,cell *size,int dim[],int numdim)
{
    ...
    if (ident==iARRAY) {
      ...
      *size=calc_arraysize(dim,numdim,0);
      ...
      for (tablesize=calc_arraysize(dim,numdim-1,0); tablesize>0; tablesize--)
        litadd(0);
      adjust_indirectiontables(dim,numdim,0,0,curlit,NULL);
    } /* if */
    return;
  } /* if */
  ...
  if (ident==iVARIABLE) {
    ...
    assert(numdim>0);
    if (numdim==1) {
      *size=initvector(ident,tag,dim[0],FALSE,NULL);
    } else {
      ...
      initarray(ident,tag,dim,numdim,0,curlit,counteddim,&lastdim,&errorfound);
       ...
       */
      adjust_indirectiontables(dim,numdim,0,0,curlit,&lastdim);
                      ...
                      int *errorfound)
{
  ...
  totalsize=0;
  needtoken('{');
  for (idx=0,abort=FALSE; !abort; idx++) {
    ...
    if (cur+2<numdim) {
      ...
      dsize=initvector(ident,tag,dim[cur+1],TRUE,errorfound);
...
static void decl_enum(int vclass)
{
  ...
  do {
    ...
    idxtag=sc_addtag(NULL);             /* optional explicit item tag */
    tok=lex(&val,&str);                 /* read in (new) token */
    if (tok!=tSYMBOL)
      error(20,str);                    /* invalid symbol name */
    strcpy(constname,str);              /* save symbol name */
    ...
    /* add_constant() must give fatal error 103 on error, so the symbol must
     * be valid if we arrived here
     */
    assert(sym!=NULL);
    /* set the item tag, for use in indexing arrays */
    sym->x.idxtag=idxtag;
    /* ??? also store the item size in the symbol structure */
    if (multiplier==1)
      value+=size;
    else
...
static int operatorname(char *name)
{
  ...
  default:
    name[0]='\0';
    error(61);          /* operator cannot be redefined (or bad operator name) */
    return 0;
  } /* switch */
  ...
  for (idx=0; idx<argcnt && arglist[idx].ident!=0; idx++) {
    if ((arglist[idx].hasdefault & uSIZEOF)!=0 || (arglist[idx].hasdefault & uTAGOF)!=0) {
      ...
      if (altidx>=argcnt) {
        ...
        assert(arglist[idx].defvalue.size.level==0 || (arglist[idx].hasdefault & uSIZEOF)!=0);
...
static void dump_referrers(symbol *root,FILE *log,char *sourcefile)
{
  ...
  for (sym=root->next; sym!=NULL; sym=sym->next) {
    ...
    for (i=0; i<sym->numrefers; i++) {
      if ((ref=sym->refer[i])!=NULL)
        fprintf(log,"\t\t\t<referrer>%s</referrer>\n",funcdisplayname(symname,ref->name));
    } /* for */
    fprintf(log,"\t\t</symbol>\n");
  } /* for */
  fprintf(log,"\t</constants>\n\n");

  fprintf(log,"\t<variables>\n");
  for (sym=root->next; sym!=NULL; sym=sym->next) {
    if (sym->parent!=NULL)
      continue;                 /* hierarchical data type */
    ...
    if ((sym->usage & uPUBLIC)!=0)
      fprintf(log,"\t\t\t<attribute value=\"public\"/>\n");
    for (i=0; i<sym->numrefers; i++) {
      if ((ref=sym->refer[i])!=NULL)
        fprintf(log,"\t\t\t<referrer>%s</referrer>\n",funcdisplayname(symname,ref->name));
    } /* for */
  ...
  for (sym=root->next; sym!=NULL; sym=sym->next) {
    ...
    if ((sym->usage & uNATIVE)!=0)
      fprintf(log,"\t\t\t<attribute value=\"native\"/>\n");
    if ((sym->usage & uPUBLIC)!=0)
      fprintf(log,"\t\t\t<attribute value=\"public\"/>\n");
    if (strcmp(sym->name,uMAINFUNC)==0)
      fprintf(log,"\t\t\t<attribute value=\"entry\"/>\n");
    if ((sym->usage & uNATIVE)==0)
...
#if !defined SC_LIGHT
static long max_stacksize_recurse(symbol *sym,long basesize)
{
  long size,maxsize;
  int i;
  ...
  for (i=0; i<sym->numrefers; i++) {
    if (sym->refer[i]!=NULL) {
      ...
      size=max_stacksize_recurse(sym->refer[i],0L);
      if (size<0)
...
static long max_stacksize(symbol *root)
{
  ...
  for (sym=root->next; sym!=NULL; sym=sym->next) {
    ...
    size=max_stacksize_recurse(sym,0L);
    if (size<0)
...
static constvalue *insert_constval(constvalue *prev,constvalue *next,char *name,cell val,
                                   short index)
{
  constvalue *cur;
...
static void test(int label,int parens,int invert)
{
  ...
  if (lval.ident==iARRAY || lval.ident==iREFARRAY) {
    const char *ptr=(lval.sym->name!=NULL) ? lval.sym->name : "-unknown-";
...
static void doswitch(void)
{
  ...
  needtoken('(');
  doexpr(TRUE,FALSE,FALSE,TRUE,NULL,FALSE);   /* evaluate switch expression */
  needtoken(')');
