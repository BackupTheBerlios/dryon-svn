 ...
 *
 *  Version: $Id: sc3.c,v 1.1 2004/04/05 15:23:38 anthalir Exp $
 */
#include <assert.h>
#include <stdio.h>
...
static void callfunction(symbol *sym,int matchparanthesis);
                   ...
                   value *lval1,value *lval2)
{
  ...
  if (oper) {
    ...
    if (lval1->ident==iARRAY || lval1->ident==iREFARRAY) {
      const char *ptr= (char*)(lval1->sym!=NULL) ? lval1->sym->name : "-unknown-";
      error(33,ptr);                    /* array must be indexed */
    } else if (lval2->ident==iARRAY || lval2->ident==iREFARRAY) {
      const char *ptr= (char*)(lval2->sym!=NULL) ? lval2->sym->name : "-unknown-";
      error(33,ptr);                    /* array must be indexed */
    } /* if */
...
static int hier13(value *lval)
{
  ...
  if (matchtoken('?')) {
    ...
    if (array1 && !array2) {
      const char *ptr= (char*)(lval->sym->name!=NULL) ? lval->sym->name : "-unknown-";
      error(33,ptr);            /* array must be indexed */
    } else if (!array1 && array2) {
      const char *ptr= (char*)(lval2.sym->name!=NULL) ? lval2.sym->name : "-unknown-";
      error(33,ptr);            /* array must be indexed */
    } /* if */
...
static void callfunction(symbol *sym,int matchparanthesis)
{
  ...
  assert(curfunc!=NULL);
  if (curfunc->x.stacksize<declared+nargs+heapalloc+1)
    curfunc->x.stacksize=declared+nargs+heapalloc+1;
}
