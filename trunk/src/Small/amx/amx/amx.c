 ...
 *
 *  Version: $Id: amx.c,v 1.5 2004/07/10 22:04:20 anthalir Exp $
 */
...
/** EDIT **/
int AMXAPI amx_NumLibraries(AMX *amx, int *number)
{
  AMX_HEADER *hdr=(AMX_HEADER *)amx->base;
  assert(hdr!=NULL);
  assert(hdr->magic==AMX_MAGIC);
  assert(hdr->natives<=hdr->libraries);
  *number=NUMENTRIES(hdr,libraries,pubvars);
  return AMX_ERR_NONE;
}

int AMXAPI amx_GetLibrary(AMX *amx, int index, char *libname)
{
  AMX_HEADER *hdr;
  AMX_FUNCSTUB *func;
  ...
  assert(hdr->magic==AMX_MAGIC);
  assert(hdr->natives<=hdr->libraries);
  if (index>=(cell)NUMENTRIES(hdr,libraries,pubvars))
    return AMX_ERR_INDEX;

  func=GETENTRY(hdr,natives,index);
  strcpy(libname,GETENTRYNAME(hdr,func));
  return AMX_ERR_NONE;
}
/****/
...
int AMXAPI amx_Init(AMX *amx,void *program)
{
  AMX_HEADER *hdr;
    ...
    #if defined _Windows
      typedef int (_FAR *AMX_ENTRY)(AMX _FAR *amx);
      HINSTANCE hlib;
    #elif defined LINUX
